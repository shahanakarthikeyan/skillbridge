...
def run_agent(resume_text: str, jd_text: str):
    ...
    reasoning_steps.append({"step": "Tool result: build_learning_pathway", "reasoning": f"Built {len(pathway)} modules"})
    ...
    try:
        reflection = client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": f"One sentence summary: {len(resume_skills)} resume skills, {len(gap_result['gaps'])} gaps, {len(pathway)} courses recommended."
            }]
        )
        agent_summary = reflection.choices[0].message.content
    except:
        agent_summary = f"Found {len(resume_skills)} skills, {len(gap_result['gaps'])} gaps, {len(pathway)} courses recommended."
